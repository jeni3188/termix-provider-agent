...
def main():
    actual_phase90_source_digest = sha256_file(PHASE90_SOURCE)

    with tempfile.TemporaryDirectory(prefix="phase91-regression-") as td:
        tmp = Path(td)

        tmp_output = tmp / PHASE90_OUTPUT.name
        tmp_checkpoint = tmp / PHASE90_CHECKPOINT.name

        shutil.copy2(PHASE90_OUTPUT, tmp_output)
        shutil.copy2(PHASE90_CHECKPOINT, tmp_checkpoint)

        module = load_module()

        module.OUT = tmp
        module.PHASE90_OUTPUT = tmp_output
        module.PHASE90_CHECKPOINT = tmp_checkpoint
        module.PHASE90_SOURCE = PHASE90_SOURCE
        module.EXPECTED_PHASE90_SOURCE_DIGEST = actual_phase90_source_digest
        ...
        # R4 previous phase mutation
        mutated = dict(baseline_output)
        mutated["previous_phase"] = 88
        tmp_output.write_text(json.dumps(mutated))
        expect_reject(module)
        print("R4 previous phase mutation: PASS")
        tmp_output.write_text(json.dumps(baseline_output))

        # R5 source digest mutation
        original_expected = module.EXPECTED_PHASE90_SOURCE_DIGEST
        module.EXPECTED_PHASE90_SOURCE_DIGEST = "f" * 64
        expect_reject(module)
        print("R5 source digest mutation: PASS")
        module.EXPECTED_PHASE90_SOURCE_DIGEST = original_expected

        # R6 execution authorization
        mutated = dict(baseline_output)
        ...
        # R10 checkpoint finality mutation
        mutated = dict(baseline_checkpoint)
        mutated["finality_digest"] = "1" * 64
        tmp_checkpoint.write_text(json.dumps(mutated))
        expect_reject(module)
        print("R10 checkpoint finality mutation: PASS")
