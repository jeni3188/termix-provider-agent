...
PHASE59_TYPE = (
    "AACP_EVIDENCE_CHAIN_HEALTH_FINALITY_STABILITY_HISTORY_CONTINUITY_DRIFT_CONTINUITY_STABILITY_HISTORY_CONTINUITY_VERIFY"
)
...
PHASE59_PATH = os.path.join(
    "provider-output", "aacp-observer",
    "latest-aacp-evidence-chain-health-finality-stability-history-continuity-drift-continuity-stability-history-continuity-verify.json",
)
...
CHECKPOINT_PATH = os.path.join(
    BASE,
    "aacp-evidence-chain-health-finality-stability-history-"
    "continuity-drift-continuity-stability-history-continuity-semantic-checkpoint.json",
)

OUTPUT_PATH = os.path.join(
    BASE,
    "latest-aacp-evidence-chain-health-finality-stability-history-"
    "continuity-drift-continuity-stability-history-continuity-semantic-verify.json",
)


# Phase60 must never write to any upstream Phase59 artifact.
if OUTPUT_PATH == PHASE59_PATH:
    raise RuntimeError("PHASE60_OUTPUT_PATH_COLLIDES_WITH_PHASE59")

if CHECKPOINT_PATH == os.path.join(
    BASE,
    "aacp-evidence-chain-health-finality-stability-history-"
    "continuity-drift-continuity-stability-history-continuity-checkpoint.json",
):
    raise RuntimeError("PHASE60_CHECKPOINT_PATH_COLLIDES_WITH_PHASE59")
...
def validate_phase59(obj):
    errors = []

    if not isinstance(obj, dict):
        return ["phase59_output_not_object"]

    if obj.get("type") != PHASE59_TYPE:
        errors.append("phase59_type_invalid")

    if obj.get("state") != "VERIFIED_READ_ONLY":
        errors.append("phase59_state_invalid")

    readiness = obj.get("readiness")
    if not isinstance(readiness, dict):
        errors.append("phase59_readiness_missing")
        readiness = {}

    if readiness.get("ready") is not True:
        errors.append("phase59_ready_invalid")

    verification = obj.get("verification")
    if not isinstance(verification, dict):
        errors.append("phase59_verification_missing")
        verification = {}

    if verification.get("valid") is not True:
        errors.append("phase59_valid_invalid")

    if obj.get("executionAuthorized") is not False:
        errors.append("phase59_execution_authorized_invalid")

    safety = obj.get("safety")
    if not isinstance(safety, dict):
        errors.append("phase59_safety_missing")
        safety = {}

    side_effects = obj.get("sideEffects")
    if not isinstance(side_effects, dict):
        errors.append("phase59_side_effects_missing")
        side_effects = {}

    if side_effects.get("networkAccess") is not False:
        errors.append("phase59_network_access_invalid")

    if side_effects.get("walletAccess") is not False:
        errors.append("phase59_wallet_access_invalid")

    if safety.get("signingPerformed") is not False:
        errors.append("phase59_signing_invalid")

    if safety.get("broadcastPerformed") is not False:
        errors.append("phase59_broadcast_invalid")

    if safety.get("submissionPerformed") is not False:
        errors.append("phase59_submission_invalid")

    if verification.get("checkpoint") != "VERIFIED":
        errors.append("phase59_checkpoint_invalid")

    digest_value = verification.get("continuityDigest")
    if not isinstance(digest_value, str) or len(digest_value) != 64:
        errors.append("phase59_continuity_digest_invalid")
    else:
        try:
            int(digest_value, 16)
        except ValueError:
            errors.append("phase59_continuity_digest_not_hex")

    return errors

def semantic_projection(obj):
    readiness = obj.get("readiness", {}) if isinstance(obj, dict) else {}
    verification = obj.get("verification", {}) if isinstance(obj, dict) else {}
    safety = obj.get("safety", {}) if isinstance(obj, dict) else {}
    side_effects = obj.get("sideEffects", {}) if isinstance(obj, dict) else {}
...
def phase59_gate():
    obj, error = load_json(PHASE59_PATH)
    if error:
        return None, ["phase59_output_unreadable:" + str(error)]

    errors = validate_phase59(obj)
    if errors:
        return None, errors
...
def main():
    phase59_obj, phase59_errors = phase59_gate()
    if phase59_errors:
        output = build_output(
        ...
        print("STATE: UNVERIFIED_READ_ONLY")
        print("READY: False")
        print("VALID: False")
        print("ERROR COUNT:", len(phase59_errors))
        return 1
    ...
    semantic_continuity_digest = semantic_digest(phase59_obj)
    ...
    if checkpoint is None:
        checkpoint_payload = {
            ...
            "continuityDigest": continuity_digest,
            "semanticDigest": semantic_continuity_digest,
        }
    ...
    if checkpoint.get("semanticDigest") != semantic_continuity_digest:
        checkpoint_errors.append("CHECKPOINT_SEMANTIC_DIGEST_MISMATCH")
